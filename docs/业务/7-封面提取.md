...
└── mongoTemplate.updateFirst(...)                    // 只更新 video.coverId（字段级）
```

> **注意**：结尾必须用字段级 `updateFirst` 写 `video.coverId`，不能用 `mongoTemplate.save(video)`。
> 截帧轮询同步阻塞最长 3 分钟，期间转码回调可能已把 `video.status` 更新为 READY；
> 若用进入时的旧快照整文档 save，会把 READY 覆盖回 TRANSCODING，播放页报"视频尚未准备好"
> （2026-09-18 线上案例：watchId 2UGS25）。
...
| 类 | 路径 |
|----|------|
| CoverService | `video/src/main/java/com/github/makewheels/video2022/cover/CoverService.java` |
...
| CoverRepository | `video/src/main/java/com/github/makewheels/video2022/cover/CoverRepository.java` |
