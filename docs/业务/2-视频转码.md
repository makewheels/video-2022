...
零云转码成本通道：客户端用本机 FFmpeg 转好 HLS 后回传，服务端只做登记，全程**不调用** MPS（含截帧）与云函数。

- **开关**：创建视频时 `transcodeMode = LOCAL`（`Video.transcodeMode`，默认 `AUTO`）。`RawFileService.onRawFileUploadFinish` 在 `LOCAL` 模式下跳过 `launchTranscode`，视频停留在 `PREPARE_TRANSCODING` 等待客户端回传。
- **provider**：`TranscodeProvider.LOCAL = "LOCAL_TRANSCODE"`，封面 `CoverProvider.LOCAL = "LOCAL_COVER"`。
- **服务端**：`LocalTranscodeService` / `LocalCoverService`，对外接口见 [转码接口文档](../api/5-转码接口.md#本地转码接口local)。
- **登记复用**：`finishTranscode` 把 Transcode 置 `FINISHED` 后直接调用 `TranscodeCallbackService.onTranscodeFinish`，与云端通道共用同一套 ts 登记/码率计算/就绪逻辑——本地转码对回调侧完全透明。
- **状态语义**：`Transcode.isFinishStatus()/isSuccessStatus()` 对 `LOCAL` 以 `status == FINISHED` 判定，确保多档分辨率只有都回传完成才会让视频 `READY`。
- **客户端**：`video-cli video upload-local`（ffprobe 探测 → 创建(LOCAL) → 上传原片 → 逐档本机 ffmpeg 切 HLS 上传 → finish → 本地截帧封面）。

```mermaid
flowchart LR
    A[ffprobe 探测源片] --> B[create transcodeMode=LOCAL]
    B --> C[上传原片到 OSS]
    C --> D[rawFileUploadFinish 服务端不转码]
    D --> E{每档分辨率}
    E --> F[createTranscode 拿凭证]
    F --> G[本机 ffmpeg 切 HLS]
    G --> H[m3u8+ts 直传 OSS outputDir]
    H --> I[finishTranscode 登记]
    I --> E
    E -->|全部完成| J[视频 READY]
```

**约束对齐**：m3u8 以相对文件名引用 ts（FFmpeg `-f hls` 默认），ts 命名 `{transcodeId}-%05d.ts`，m3u8/ts 落在 `OssPathUtil.getM3u8Key` 算出的目录下，从而被 `M3u8Util` 与回调侧的 OSS 列举原样解析。
